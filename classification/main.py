...
if __name__ == '__main__':
    ...
    train_poetry_list, train_label_list, test_poetry_list, test_label_list = classification_word2vec_data_set(
        poetry_type, False)
    ...
    with tf.Session() as sess:
        ...
        for train_time in range(3):
            ...
            for i in range(int(len(test_poetry_list) // batch_size)):
                ...
                answer.append(sess.run(tf.argmax(model.output, 1),
                               feed_dict={model.data: test_poetry_list[i * batch_size:(i + 1) * batch_size]}))
                # answer.append(sess.run(tf.argmax(model.label, 1),
